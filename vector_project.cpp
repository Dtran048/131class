...
template<typename T> class Inventory{
    public:
        ...
        void addNewItem(Item<std::string> a){
        ...
        void increaseQuantity(std::string a, int b){
            for(int i=0; i<items.size(); i++){
                if (a == items[i].name){
                    items[i].quantity = b;
                }
        ...
        void updateItem(std::string a,std::string b,std::string c,int d){
            for(int i=0; i<items.size(); i++){
                if (a == items[i].name){
                    items[i].expiration = b;
                    items[i].category = c;
                    items[i].quantity = d;
                    return;
                }
        ...
        void Total(){
            int numfood = 0;
            for(int i=0; i<items.size(); i++){
                numfood += items[i].quantity;
            }
             std::cout<< "there is " << numfood << " items in the inventory." <<std::endl;
        }
        void searchItem(std::string a){
            for(int i=0; i<items.size(); i++){
                if (a == items[i].name){
                    throw ("Item found");
                    return;
...
template<typename T>class AppointmentSystem{
        ...
        void schedule(Appointment<std::string> a){
            for(int i=0; i<ap.size(); i++){
                if (a.c_name == ap[i].c_name){
        ...
        void Total_appointments(std::string a,std::string b){
            int numapp = 0;
            for(int i=0; i<ap.size(); i++){
                if ((a == ap[i].ap_date) && (b==ap[i].ap_time)){
                    numapp += 1;
                }
            }
            std::cout<< "There are " << numapp << " appointments on " << a << " at "<< b << ". " <<std::endl;
        }
        void removeRecent(){
            ap.pop_back();
        }
